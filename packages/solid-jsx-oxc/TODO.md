...
- ~~SSR nested spread element allocated a spurious hydration key, drifting the counter~~ - For JSX like `<div class="split"><button {...rest}><Icon/>{label}</button></div>` (a `SplitButton` whose `<button>` is `_el$.firstChild`, not the template root), the SSR transform emitted `ssrElement("button", …, true)` — the `needsId` (4th) arg unconditionally `context.hydratable && options.hydratable`. On the DOM side the client reaches the nested `<button>` by `firstChild` walking from the wrapper `<div>`'s single `getNextElement` and `spread()` on it advances no hydration counter, so requesting a key server-side over-allocated one `getNextContextId()` and drifted the SSR counter one ahead of the DOM counter. Every subsequent key — the nested `<Icon>`'s `<svg>` first — then pointed at the wrong SSR node, surfacing as `Hydration Mismatch. Unable to find DOM nodes for hydration key … <svg> at Icon`. `@gothab/ui`'s Button never hit it because its `<button>` *is* the template root (`needsId` correctly `true`). Babel's `src/ssr/element.js:~608` gates the same arg on `Boolean(topLevel && config.hydratable)`. Fixed in `crates/ssr/src/element.rs` by threading `top_level` into `transform_element_with_spread` (previously dropped at the `has_spread` short-circuit) and gating the `ssrElement` 4th arg on `top_level && context.hydratable && options.hydratable`. Pinned by `ssr_regression_nested_spread_element_has_no_hydration_key` (nested → `false`) and `ssr_regression_root_spread_element_keeps_hydration_key` (root → `true`) in `tests/transform_tests.rs`.

## New Features

- **Compile-time hydration slot-order check** — SolidJS SSR allocates hydration keys in *execution order* on the server (eager `children()`/`createMemo` resolve at their declaration point; `ssrHydrationKey()` fires as the template string is assembled) but the client consumes them in *DOM-walk order*. A component that resolves keyed element slots in an order differing from their DOM order desyncs the two counters, surfacing as `Hydration Mismatch. Unable to find DOM nodes for hydration key …` — hit twice in `@gothab/ui` (ListItem, then Alert). This is the stock dom-expressions contract (reproduces identically under `babel-plugin-jsx-dom-expressions`), so it can't be fixed in codegen. Instead a read-only semantic pass (`src/hydration_order.rs`) runs on the pristine AST inside `transform_internal` (gated on `hydratable`) and flags two source shapes: (1) **out-of-order `children()`** — element slots resolved through `children()` whose render order differs from declaration order (deterministic; dependency-guarded so a reorder that would break a slot referencing an earlier one is not flagged); (2) **mixed hoisted + inline** — a `children()`-resolved slot rendered *after* an inline element insert (heuristic — the compiler can't prove an inline `{local.x}` is element-valued vs a string, so it can over-report; the fix is always safe). Diagnostics carry a fix-it `help` naming the exact rearrange and are threaded through NAPI (`TransformResult.diagnostics`) to the Vite/Rolldown/Bun plugins, which fail the build on `severity: "error"`. Controlled by `hydration_order_check` (`TransformOptions`) / `hydrationOrderCheck` (NAPI + plugin option): `"error"` (default, fatal), `"warn"`, or `"off"`. Covered by `tests/hydration_order_tests.rs`. Deferred (v1): only `children()` is tracked (not `createMemo`); the `children` import is matched by local name; render order is tracked only within a single function's own returned JSX.
